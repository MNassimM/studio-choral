...
} from "@/features/admin/works/server/sellability/product-activation";
import {
  ActionError,
  toFailure,
  type ActionResult,
} from "@/features/admin/works/server/work-action-errors";
import {
  workFormSchema,
  type WorkFormValues,
...
import {
  ...
  uniqueSlugs,
  type VoiceRow,
} from "@/features/admin/works/server/work-products";
import { revalidateCatalog } from "@/features/admin/works/server/work-revalidation";
import {
  deleteStoredObjects,
  storePendingTracks,
} from "@/features/admin/works/server/work-track-storage";
import {
  deleteCoverObject,
  storePendingCover,
} from "@/features/admin/works/server/work-cover-storage";
import { prisma } from "@/server/db/prisma";
import { nextPublishedAt } from "@/features/work/domain/publication";
...
export async function publishWork(workId: string): Promise<ActionResult> {
  ...
  await prisma.work.update({
    ...
    data: {
      isPublished: true,
      // Figée à la première mise en vente. Voir features/work/domain/publication.ts pour
