 ...
 * libellé existe dans messages/*.json ("workLanguage.*"). Dérivé directement
 * de fr.json (source de vérité des clés) plutôt que dupliqué à la main.
 ...
 */
const KNOWN_WORK_LANGUAGE_CODES = new Set(Object.keys(frMessages.workLanguage));

export function isKnownWorkLanguageCode(
  code: string,
): code is keyof typeof frMessages.workLanguage {
